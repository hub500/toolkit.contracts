...
uint64_t get_zero_clock_timestamp(const time_point& time) 
{  
    uint64_t time64 = time.sec_since_epoch();
    uint64_t zero_clock_timestamp = time64 - time64%SECONDS_PER_DAY;
    // uint64_t zero_clock_timestamp = time64 - time64%86400;
    return zero_clock_timestamp;
}

time_point get_zero_clock(const time_point& time) 
{  
    uint64_t time64 = time.sec_since_epoch();
    uint64_t zero_clock_timestamp = time64 - time64 % SECONDS_PER_DAY;
    return time_point(seconds(zero_clock_timestamp));
}

uint64_t cal_diff_days( const time_point_sec& a, const time_point_sec& b){
    if ( a < b) return 0;
    int64_t days = int( a.sec_since_epoch() / SECONDS_PER_DAY) - int( b.sec_since_epoch() / SECONDS_PER_DAY );
    return days;
}

uint64_t cal_days( const time_point_sec& last_time){

    // int64_t days = int( current_time_point().sec_since_epoch() / SECONDS_PER_DAY) - int( last_time.sec_since_epoch() / SECONDS_PER_DAY );
    return cal_diff_days(current_time_point(),last_time);
}
